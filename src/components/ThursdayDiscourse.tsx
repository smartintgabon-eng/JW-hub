...
import { ChevronRight, Send, BookOpen, Calendar } from 'lucide-react';
...
const ThursdayDiscourse: React.FC<ThursdayDiscourseProps> = ({ settings, setGlobalLoadingMessage }) => {
  ...
  const [data, setData] = useState({ theme: '', time: '10min', refs: '' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(`Erreur lors de la génération du discours de Jeudi: ${error.message}`);
    } finally {
      setGlobalLoadingMessage(null);
    }
  };
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-black uppercase flex items-center gap-3">
              <div className="p-2 bg-[var(--btn-color)]/20 rounded-xl">
                <Calendar size={20} className="text-[var(--btn-color)]" />
              </div>
              Étape 1 : Thème & Durée
            </h3>
            ...
            <div className="space-y-3">
              ...
              <input
                type="text"
                placeholder="Ou entrez une durée personnalisée (ex: 8min)"
                value={!['5min', '10min', '15min'].includes(data.time) ? data.time : ''}
