  ...
  if (selectedStudy) {
    return (
      <div className="animate-in fade-in slide-in-from-right-4 duration-500 flex flex-col h-full max-w-4xl mx-auto pb-24">
        ...
        <article className="bg-white/5 rounded-[3.5rem] p-8 md:p-16 shadow-2xl border border-white/10 relative">
          <header className="mb-16 text-center">
             ...
             <h1 className="text-4xl md:text-6xl font-black leading-[1.1] mb-6" style={{ color: 'var(--text-color)' }}>
                {selectedStudy.title}
             </h1>
          ...
          <div className="space-y-10 font-serif text-xl md:text-2xl leading-relaxed" style={{ color: 'var(--text-color)' }}>
            {selectedStudy.content.split('\n').map((line, idx) => {
              ...
              if (trimmed.includes('PARAGRAPHE') || trimmed.includes('RÉVISION')) {
                return (
                  <h3 key={idx} className="text-2xl md:text-4xl font-black pt-12 border-t border-white/10 mt-16 first:mt-0 first:pt-0 first:border-0" style={{ color: 'var(--btn-color)' }}>
                    {trimmed}
                  </h3>
                );
              }
              ...
              if (trimmed.startsWith('VERSET')) {
                return (
                  <div key={idx} className="p-8 my-8 bg-black/20 rounded-3xl border-l-8 border-[var(--btn-color)] italic shadow-inner">
                    <span className="block mb-2 text-xs font-black uppercase opacity-50 not-italic">Texte Biblique</span>
              ...
              if (trimmed.startsWith('RÉPONSE') || trimmed.startsWith('COMMENTAIRE') || trimmed.startsWith('APPLICATION')) {
                ...
                return (
                  <p key={idx} className="mt-8 font-sans">
                    <span className="font-black uppercase text-xs mr-3 px-3 py-1 bg-[var(--btn-color)] rounded-md" style={{ color: 'var(--btn-text)' }}>
                      {parts[0]}
                    </span>
              ...
              return <p key={idx} className="opacity-90">{trimmed}</p>;
            })}
          </div>
        </article>
      </div>
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center space-x-6 mb-12">
        ...
        <div>
          <h2 className="text-4xl font-black tracking-tight">Historique</h2>
          <p className="opacity-50 font-bold">Vos préparations enregistrées.</p>
        </div>
      </div>
      ...
      {history.length === 0 ? (
        <div className="py-40 bg-white/5 border-2 border-white/10 border-dashed rounded-[4rem] text-center flex flex-col items-center">
          ...
          <p className="text-2xl font-black opacity-30">Aucun historique</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {history.map((study) => (
            <div
              ...
              className="bg-white/5 border border-white/10 rounded-[3rem] p-10 hover:bg-white/10 transition-all cursor-pointer group shadow-2xl"
            >
              ...
              <h3 className="font-black text-2xl mb-6 leading-tight line-clamp-2">{study.title}</h3>
              <div className="flex items-center space-x-3 text-[11px] font-black uppercase opacity-40">
