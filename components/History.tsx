      ...
      sections: [{
        children: [
          ...
          ...study.content.split('\n').map(line => {
            ...
            if (trimmed.startsWith('#')) return new Paragraph({ text: trimmed.substring(1).trim(), heading: HeadingLevel.HEADING1, spacing: { before: 240, after: 120 } });
  ...
  const exportToPdf = (study: GeneratedStudy) => {
    ...
    lines.forEach(line => {
      ...
      if (trimmed.startsWith('#')) {
        ...
        const textLines = doc.splitTextToSize(rest.join(':').trim(), pageWidth - 2 * margin - doc.getStringUnitWidth(`${label}: `) * doc.internal.getFontSize());
        doc.text(textLines, margin + doc.getStringUnitWidth(`${label}: `) * doc.internal.getFontSize(), y, { align: 'left' });
        y += (textLines.length * 5) + 3;
