...
import { Search, Link as LinkIcon, Calendar, Loader2, Globe, Check, Info, ShieldCheck, AlertTriangle } from 'lucide-react';
...
const StudyTool: React.FC<Props> = ({ type, onGenerated, settings }) => {
  ...
  // Persistance robuste des saisies pour éviter les pertes
  useEffect(() => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div className="bg-white/5 border border-white/10 rounded-[2rem] p-8 space-y-8 shadow-2xl relative overflow-hidden">
        <div className="space-y-3">
          <label className="text-[10px] font-black uppercase opacity-40 ml-1 tracking-[0.2em]">Source de l'étude</label>
        ...
        {error && (
          <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm font-bold flex items-start space-x-3">
        ...
        <button
          ...
          className="w-full py-5 rounded-xl font-black uppercase tracking-widest flex items-center justify-center space-x-3 shadow-2xl active:scale-95 disabled:opacity-20 transition-all"
        >
          {loading ? <Loader2 className="animate-spin" size={24} /> : <Search size={24} />}
          <span className="text-lg">{loading ? "Recherche..." : "Lancer l'Analyse"}</span>
        </button>
      </div>
      ...
      {preview && (
        <div className="bg-white/5 border border-white/20 rounded-[2.5rem] p-10 animate-in zoom-in-95 duration-500 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex items-center space-x-3 mb-6 text-[var(--btn-color)]">
            ...
            <span className="text-xs font-black uppercase tracking-[0.3em]">Article Confirmé</span>
          </div>
          <h3 className="text-3xl font-black mb-3 leading-tight uppercase tracking-tighter">{preview.title}</h3>
          <p className="text-lg opacity-60 mb-8 font-serif italic">{preview.theme || "Thème biblique identifié"}</p>
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-6 mb-10 flex items-start space-x-4">
             ...
             <div className="space-y-1">
                <p className="font-black text-sm uppercase text-blue-200">Confirmation de génération</p>
                <p className="text-xs leading-relaxed opacity-70">
                   Les réponses seront générées automatiquement paragraphe par paragraphe en suivant l'ordre de l'article.
                   L'étude sera enregistrée dans votre <strong>historique</strong> pour un accès <strong>hors ligne</strong> permanent.
                </p>
             </div>
          </div>

          <div className="flex flex-col sm:flex-row gap-4">
            <button onClick={() => handleGenerate(true)} style={{ backgroundColor: 'var(--btn-color)', color: 'var(--btn-text)' }} className="flex-1 py-4 rounded-xl font-black text-sm uppercase tracking-widest shadow-xl active:scale-95 transition-all">Générer les réponses</button>
